...
print("=" * 70)
print("  Minerva v3 — KUYAS Özel G1 Bulucu (2025 Başına Kadar)")
print("=" * 70)
...
print(f"🔎 2025 başından günümüze tarama başlatılıyor... ({len(search_dates)} işlem günü)")

found = False
for i, day_date in enumerate(search_dates, 1):
    day_dt = pd.to_datetime(day_date)
    
    # Get last known features up to day_dt for KUYAS only
    feats = (
    ...
    if labels[0] == g1_cluster_id:
        print(f"\n🎉 KUYAS hissesi G1 kümesinde bulundu!")
        print(f"   Tarih: {str(day_date)[:10]}")
        found = True
        break

if not found:
    print(f"\n⚠️ 2025 başından bugüne KUYAS hissesi hiçbir gün G1 kümesine girmemiş.")
